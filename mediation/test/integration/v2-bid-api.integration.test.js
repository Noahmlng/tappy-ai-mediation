...
test('v2 bid API returns unified response on the single runtime path', async () => {
  ...
  try {
    ...
    if (failOpenUpstream) {
      ...
      assert.equal(bid.payload?.diagnostics?.retrievalDebug?.scoring?.sparseWeight, 0.8)
      assert.equal(bid.payload?.diagnostics?.retrievalDebug?.scoring?.denseWeight, 0.2)
      ...
      assert.equal(bid.payload?.diagnostics?.rankingDebug?.gateStrategy, 'composite_single_gate')
      assert.equal(typeof bid.payload?.diagnostics?.rankingDebug?.gateWeights, 'object')
      assert.equal(bid.payload?.diagnostics?.rankingDebug?.gateWeights?.fused, 0.55)
      assert.equal(bid.payload?.diagnostics?.rankingDebug?.gateWeights?.relevance, 0.35)
      assert.equal(bid.payload?.diagnostics?.rankingDebug?.gateWeights?.topic, 0.1)
      assert.equal(typeof bid.payload?.diagnostics?.rankingDebug?.thresholdsApplied, 'object')
      assert.equal(bid.payload?.diagnostics?.rankingDebug?.thresholdsApplied?.topicCoverage, 0.05)
      assert.equal(bid.payload?.diagnostics?.rankingDebug?.thresholdsApplied?.strict, 0.44)
      assert.equal(bid.payload?.diagnostics?.rankingDebug?.thresholdsApplied?.relaxed, 0.36)
      assert.equal(Array.isArray(bid.payload?.diagnostics?.rankingDebug?.candidates), true)
      if ((bid.payload?.diagnostics?.rankingDebug?.candidates || []).length > 0) {
        assert.equal(typeof bid.payload?.diagnostics?.rankingDebug?.candidates?.[0]?.gateScore, 'number')
        assert.equal(typeof bid.payload?.diagnostics?.rankingDebug?.candidates?.[0]?.gateComponentScores, 'object')
      }
