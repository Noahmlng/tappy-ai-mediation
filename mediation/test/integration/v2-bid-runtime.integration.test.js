...
test('v2 bid runtime: retrieval debug exposes latest-turn context and topic coverage scores', async () => {
  ...
  assert.equal(Array.isArray(result.debug?.sparseQueryTokens), true)
  assert.equal(typeof result.debug?.vectorInputTextPreview, 'string')
  assert.equal(typeof result.debug?.options?.[0]?.topicCoverageScore, 'number')
  assert.equal(typeof result.debug?.options?.[0]?.retrievalTextPreview, 'string')
  assert.equal(Array.isArray(result.debug?.options?.[0]?.queryTokensMatched), true)
  assert.equal(Array.isArray(result.debug?.options?.[0]?.bm25TermHits), true)
